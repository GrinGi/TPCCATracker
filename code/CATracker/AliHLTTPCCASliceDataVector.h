...
//    void SetHitLinkDownData( const AliHLTTPCCARow &row, const int_i &hitIndex, const int_v &value );
  ...
  private:
    enum {
      VectorSizeFactor = SimdSizeInt / SimdSizeFloat
    };
...
#else
    r[i] = row.fLinkUpData[hitIndexes[i]];
#endif
  }
  return r;
}
...
#else
    r[i] = row.fLinkDownData[hitIndexes[i]];
#endif
  }
  return r;
}
...
{
  for( unsigned int iHv = 0; iHv < std::min(indexes.size(), SimdSizeFloat); iHv++ ) {	//TODO indexes.size() - slow!
...
inline void AliHLTTPCCASliceData::CleanUsedHits( int rowIndex, bool isFirst )
{
  ...
  if ( isFirst ) {
    ...
    for ( unsigned int i = 0; i < numberOfHits; i += SimdSizeInt ) {
      const uint_v hitIndexes = /*uint_v( Vc::IndexesFromZero )*/IndexesFromZeroInt + i;
